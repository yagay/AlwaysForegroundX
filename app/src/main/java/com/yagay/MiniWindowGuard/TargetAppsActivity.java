    ...
    private void buildUi() {
        ...
        help.setText(
                MODE_FORCE_SUPPORT.equals(mode)
                        ...
                        : MODE_BACKGROUND_PLAYBACK.equals(mode)
                        ? "普通后台播放名单。App 保持普通全屏任务，切到桌面、其他 App 或锁屏时走普通后台播放保护，不要求进入系统小窗。"
                        : "小窗后台名单。App 切到后台时自动进入 OxygenOS 原生 FlexibleWindow，并缩成系统 FloatHandle 小图标；进入小窗后继续使用 5.2.0 的小窗、小图标和锁屏播放保护。");
